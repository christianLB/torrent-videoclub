// app/api/tmdb/movies/trending/route.ts
...
export async function GET(request: Request) {
  ...
  try {
    console.log(`[API TMDB Trending Movies] Fetching ${timeWindow} page ${page}`);
    const items = await tmdbDataService.getOrFetchTrendingMovies(timeWindow, page);
    ...
    console.error(`[API TMDB Trending Movies] Error fetching ${timeWindow} page ${page}:`, error);
    ...
    return NextResponse.json(
      { error: 'Internal server error while fetching trending movies.' },
      { status: 500 }
    );
  }
}
